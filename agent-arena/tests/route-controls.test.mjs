...
test('50 dollar demo starts one rules bot and refuses to overwrite a running session',async()=>{const h=harness();const r=await h.post('demo50');assert.equal(r.status,200);const s=h.state();assert.equal(s.settings.capital,50);assert.equal(s.settings.mode,'single');assert.equal(s.settings.selected,'atlas');assert.equal(s.settings.driver,'rules');assert.equal(s.running,true);assert.equal(s.trades.length,0);assert.equal((await h.post('demo50')).status,400);});
