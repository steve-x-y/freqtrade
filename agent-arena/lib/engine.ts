...
export type State = { archives?: {time:number;snapshot:string}[]; forward?: ForwardEvidence; settings: Settings; agents: Agent[]; running: boolean; lastCandle: number; lastTick: number; startedAt: number; benchmarkPrice: number; history: Point[]; trades: Trade[]; error: string | null; candles: Candle[]; price: number; tokens: number; aiCalls: number; cycle: number };
