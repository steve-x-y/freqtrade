...
import type { Quote } from './forward';
...
export async function getCandles(interval=5):Promise<Candle[]> {
  ...
  if(candles.length<50||candles.some((c,i)=>![c.time,c.open,c.high,c.low,c.close,c.volume].every(Number.isFinite)||c.open<=0||c.close<=0||c.low<=0||c.high<Math.max(c.open,c.close,c.low)||c.low>Math.min(c.open,c.close)||c.volume<0||(i>0&&c.time-candles[i-1].time!==interval*60000)))throw new Error('Market feed contains invalid candles.');
  return candles;
}
export async function getQuote():Promise<Quote>{
  ...
  return {bid,ask,requestedAt,receivedAt:Date.now()};
}

export async function getPrice():Promise<number>{const q=await getQuote();return q.bid;}
