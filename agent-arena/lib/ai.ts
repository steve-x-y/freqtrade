import { indicators, intervalMinutes, validateDecision, equity, type Agent, type Candle, type Settings } from './engine.ts';
...
export async function aiDecision(a:Agent,candles:Candle[],settings:Settings,connection:Connection){
  ...
  const prompt={market:'BTC/USD spot',timeframe:`${intervalMinutes(settings)} minutes`,asOf:new Date(candles.at(-1)!.time+intervalMinutes(settings)*60000).toISOString(),indicators:indicators(candles),recentCandles:candles.slice(-30),costs:{feeBpsPerSide:settings.feeBps,slippageBpsPerSide:settings.slippageBps},portfolio:{cash:a.cash,btc:a.quantity,equity:equity(a,candles.at(-1)!.close),costBasis:a.basis},limits:{maxExposure:settings.maxExposure,stopLoss:settings.stopLoss,maxDrawdown:settings.maxDrawdown},lastDecision:a.decision??null};
